        ...
        <div className="flex min-h-screen bg-[#020617]">
            ...
            >
                <div className="p-6 flex items-center gap-3 border-b border-white/10">
                    <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-amber-500 to-yellow-600 flex items-center justify-center font-bold text-lg text-white shadow-lg shadow-amber-500/30">FB</div>
                    <div className="flex flex-col">
                        <span className="text-xl font-bold text-white tracking-tight">FinBank</span>
                        <span className="text-[10px] text-amber-300 bg-amber-500/10 px-1.5 py-0.5 rounded">CEO workspace</span>
                    </div>
                </div>
                    ...
                    {links.map((link) => (
                        <Link
                            ...
                            className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200
                                ${isActive(link.to)
                                    ? "bg-amber-500/20 text-amber-400 shadow-[inset_0_1px_1px_rgba(255,255,255,0.1)] border border-amber-500/30"
                                    : "text-white/60 hover:bg-white/5 hover:text-white"}`}
                            aria-current={isActive(link.to) ? "page" : undefined}
                        >
                            <link.icon size={18} className={isActive(link.to) ? "text-amber-400" : "text-white/50"} />
                ...
                <div className="p-6 border-t border-white/10 flex flex-col gap-4 bg-black/20">
                    ...
                    <div className="flex items-center gap-3 p-3 rounded-xl bg-white/5 border border-white/10">
                        ...
                        <div className="flex flex-col overflow-hidden">
                            <span className="text-[10px] uppercase font-bold tracking-wider text-amber-500">CEO</span>
                            <span className="text-xs text-white/70 truncate">{user?.email}</span>
                        </div>
                    </div>

                    <button
                        className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium text-amber-400 hover:bg-amber-400/10 border border-transparent hover:border-amber-400/20 transition-all w-full"
                        onClick={logout}
                    >
            ...
            {/* ── Mobile Header ── */}
            <div className="md:hidden flex justify-between items-center p-4 bg-deepblue-950/80 backdrop-blur-xl border-b border-white/10 sticky top-0 z-50 w-full">
                <button
                ...
                <div className="flex items-center gap-2">
                    <div className="w-8 h-8 rounded-lg bg-amber-500/20 flex items-center justify-center font-bold text-xs text-amber-400 border border-amber-500/30">FB</div>
